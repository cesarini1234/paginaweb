...
{
    $bd = obtenerConexion();
    $sentencia = $bd->query("SELECT id, nombre, descripcion, precio, tema, proveedor FROM productos");
    return $sentencia->fetchAll();
}
...
function obtenerIdsDeProducto()
{
   ...
   // $sentencia = $bd->query("SELECT id, nombre, descripcion, precio FROM productos");
    ...
    $bd = obtenerConexion();
    @session_start();
    $sentencia = $bd->prepare("SELECT id FROM productos WHERE nombre = ?");
    $idSesion = intval($_GET['$nombre']);
    $sentencia->execute([$idSesion]);
    return $sentencia->fetchAll(PDO::FETCH_COLUMN);
}
